...
sub _else_pages(){
	...
	my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-cospa-job/privacy/};
	mkdir($dirname, 0755);
	my $html = &_load_tmpl("privacy.html");
	$html = &_parts_set($html);
	&_file_output("privacy/index.html",$html);

	my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-cospa-job/about/};
	mkdir($dirname, 0755);
	my $html = &_load_tmpl("about.html");
	$html = &_parts_set($html);
	&_file_output("about/index.html",$html);

	my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-cospa-job/faq/};
	mkdir($dirname, 0755);
	my $html = &_load_tmpl("faq.html");
	$html = &_parts_set($html);
	&_file_output("faq/index.html",$html);

	my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-cospa-job/contact/};
	mkdir($dirname, 0755);
...
sub _parts_set(){
	...
	my $topslider = &_load_tmpl("jobslider.html");
...
sub _file_output(){
	...
	my $file = qq{/var/www/vhosts/goo.to/httpdocs-cospa-job/}.$filename;
