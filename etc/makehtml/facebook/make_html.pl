...
sub _get_page_detail(){
	...
	my $html;
	my $url = qq{http://facebook.webk-vps.com/page.htm?type=$type&genre=$genre&page=$page&list=$list&batch=1};
print $url."\n";
	...
	if($type eq "category"){
		my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-facebook/$type/$genre/};
		mkdir($dirname, 0755);
		$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/$type/$genre/$page.html};
	}elsif($type eq "person"){
		my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-facebook/$type/$genre/};
		mkdir($dirname, 0755);
		$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/$type/$genre/$page.html};
	}else{
		$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/$type/$page.html};
	}

	if($list){
		if($type eq "category"){
			my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-facebook/list/$type/$genre/};
			mkdir($dirname, 0755);
			$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/list/$type/$genre/$page.html};
		}elsif($type eq "person"){
			my $dirname = qq{/var/www/vhosts/goo.to/httpdocs-facebook/list/$type/$genre/};
			mkdir($dirname, 0755);
			$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/list/$type/$genre/$page.html};
		}else{
			$filename = qq{/var/www/vhosts/goo.to/httpdocs-facebook/list/$type/$page.html};
		}
	}
...
sub _top(){
	...
	# 新着
	my $list;
	$list = &_make_list($dbh, " order by likecnt desc limit 30 ");
