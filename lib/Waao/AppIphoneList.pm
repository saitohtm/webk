...
sub dispatch(){
	...
	my $html = &_load_tmpl("good_iphonelist.html");
	$html =~s/<!--PAGE-->/$page/g;
	...
	my $list2 = &_make_list($self, qq{ order by releaseDate desc, eva desc limit $start, $pagemax },$start);
	$html =~s/<!--LIST-->/$list2/g;

	my $pager .= &_pager($page,"good-iphone-app");
