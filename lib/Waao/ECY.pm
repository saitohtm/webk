...
sub dispatch(){
	...
	my $sth = $self->{dbi}->prepare(qq{select `itemCode`,`itemName`,`catchcopy`,`itemPrice`,`itemCaption`,`itemUrl`,`affiliateUrl`,`smallImageUrl`,`mediumImageUrl`,`taxFlag`,`postageFlag`,`creditCardFlag`,`shopOfTheYearFlag`,`asurakuFlag`,`reviewCount`,`reviewAverage`,`shopName`,`shopCode`,`shopUrl`,`genreId`,titlestr from yahoo_item where itemCode = ? });
...
#	$name = $catchcopy if($catchcopy);
	...
	my $keys = qq{通販,送料無料,yahoo,ブランド,ショッピング,ファッション,価格,$keywords};
	my $desc = qq{$itemName };
...
my $file = qq{/var/www/vhosts/waao.jp/etc/makeshophtm/tmpl_yahoo/$tmpl};
my $fh;
my $filedata;
open( $fh, "<", $file );
...
#	$name = $catchcopy if($catchcopy);
	my $title = qq{$titlestr yahooID:$itemcode ブランド通販};
	my $keys = qq{通販,送料無料,yahoo,ブランド,価格,$keywords};
	my $desc = qq{$itemName };
