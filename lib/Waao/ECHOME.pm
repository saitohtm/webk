...
sub dispatch(){
	...
	my ($itemCode,$itemName,$catchcopy,$itemPrice,$itemCaption,$itemUrl,$affiliateUrl,$smallImageUrl,$mediumImageUrl,$taxFlag,$postageFlag,$credithomedFlag,$shopOfTheYearFlag,$asurakuFlag,$reviewCount,$reviewAverage,$shopName,$shopCode,$shopUrl,$genreId,$titlestr);
	my $sth = $self->{dbi}->prepare(qq{select `itemCode`,`itemName`,`catchcopy`,`itemPrice`,`itemCaption`,`itemUrl`,`affiliateUrl`,`smallImageUrl`,`mediumImageUrl`,`taxFlag`,`postageFlag`,`creditCardFlag`,`shopOfTheYearFlag`,`asurakuFlag`,`reviewCount`,`reviewAverage`,`shopName`,`shopCode`,`shopUrl`,`genreId`,titlestr from yahoo_item_home where itemCode = ? });
...
#	$name = $catchcopy if($catchcopy);
	...
	$title .=qq{ };
	my $keys = qq{インテリア通販,送料無料,yahoo,インテリア,ショッピング,家具,価格,$keywords};
	my $desc = qq{$itemName };
...
my $file = qq{/var/www/vhosts/waao.jp/etc/makeshophtm/tmpl_yahoo_home/$tmpl};
my $fh;
my $filedata;
open( $fh, "<", $file );
...
#	$name = $catchcopy if($catchcopy);
	my $title = qq{$titlestr yahooID:$itemcode インテリア・家具通販};
	my $keys = qq{インテリア通販,送料無料,yahoo,インテリア,価格,$keywords};
	my $desc = qq{$itemName };
