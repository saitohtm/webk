...
<form action="http://www.google.co.jp/cse" id="cse-search-box" target="_blank">
  <div>
<img src="http://goo.to/img/cn11.gif" width=46 height=52>
「<a href="/person$meikantype/">$meikanname名鑑</a>」
    <input type="hidden" name="cx" value="partner-pub-2078370187404934:nqwfgk9rnxl" />
    <input type="hidden" name="ie" value="Shift_JIS" />
...
sub _slist(){
	...
	my $slist = qq{<h2>$meikan 50音検索</h2>};
	...
	$slist .= qq{<a href="/person$meikantype-hi/" title="$meikan ひ行">ひ</a> };
	$slist .= qq{<a href="/person$meikantype-hu/" title="$meikan ふ行">ふ</a> };
	$slist .= qq{<a href="/person$meikantype-he/" title="$meikan へ行">へ</a> };
	$slist .= qq{<a href="/person$meikantype-ho/" title="$meikan ほ行">ほ</a> };
	$slist .= qq{<br>};
	...
	$slist .= qq{<br>};
	$slist .= qq{<a href="/person$meikantype-wa/" title="$meikan わ行">わ</a> <br>};
...
sub _top_dsp(){
	...
	my $start = 0 + $page * 20;
	my $type = $self->{cgi}->param('type');
	$type = undef if($self->{cgi}->param('type') eq '50on');
	my $html;
	my $sth;
	if($type){
		$sth = $self->{dbi}->prepare(qq{ select id, inital, keyword, simplewiki from keyword where $sql_str and inital = ? order by cnt desc limit $start,20} );
		$sth->execute($type);
	}else{
		$sth = $self->{dbi}->prepare(qq{ select id, inital, keyword, simplewiki from keyword where $sql_str order by cnt desc limit $start,20} );
		$sth->execute();
	}
	my $cnt;
	my ($keyword_id, $inital, $keyword, $wikipedia);
	while(my @row = $sth->fetchrow_array) {
		($keyword_id, $inital, $keyword, $wikipedia) = @row;
		my $keyword_encode = &str_encode($keyword);
		...
		$cnt++;
		# 最初の2つは、画像を表示する
		$html .= qq{<img src="http://img.waao.jp/right06.gif" width=10 height=10><a href="/id$keyword_id/" title="$keyword">$keyword</a><br><img src="$photourl" width=95  alt="$keywordの画像"  align="left"><font color="#AAAAAA">$wikipedia</font><br clear="all" /><br>};
	}
	$page++;
	my $slist;
	my ($meikanname, $sql_str) = &_meikan_name($meikantype);
...
if($type){
print << "END_OF_HTML";
<img src="http://img.waao.jp/m2028.gif" width=48 height=9><a href="/person$meikantype-$type-p$page/">次へ</a><br>
END_OF_HTML
}else{
print << "END_OF_HTML";
...
print << "END_OF_HTML";
<img src="http://img.waao.jp/lamp1.gif" width=14 height=11><font color="#009525">人物名鑑シリーズ</font><br>
<font color="#009525">■</font><a href="/person1/" title="男性タレント名鑑">男性タレント名鑑</a><br>
<font color="#009525">■</font><a href="/person2/" title="女性タレント名鑑">女性タレント名鑑</a><br>
<font color="#009525">■</font><a href="/person3/" title="グラビアアイドル名鑑">グラビアアイドル名鑑</a><br>
<font color="#009525">■</font><a href="/person4/" title="お笑いタレント名鑑">お笑いタレント名鑑</a><br>
<font color="#009525">■</font><a href="/person6/" title="子役名鑑">子役名鑑</a><br>
<font color="#009525">■</font><a href="/person7/" title="落語家名鑑">落語家名鑑</a><br>
<font color="#009525">■</font><a href="/person8/" title="声優名鑑">声優名鑑</a><br>
<font color="#009525">■</font><a href="/person9/" title="男性アーティスト名鑑">男性アーティスト名鑑</a><br>
<font color="#009525">■</font><a href="/person10/" title="女性アーティスト名鑑">女性アーティスト名鑑</a><br>
<font color="#009525">■</font><a href="/person11/" title="モデル名鑑">モデル名鑑</a><br>
<font color="#009525">■</font><a href="/person12/" title="レースクィーン名鑑">レースクィーン名鑑</a><br>
<font color="#009525">■</font><a href="/person13/" title="女子アナウンサー名鑑">女子アナウンサー名鑑</a><br>
<font color="#009525">■</font><a href="/person14/" title="AV女優名鑑">AV女優名鑑</a><br>
<font color="#009525">■</font><a href="/person15/" title="ブログ名鑑">ブログ名鑑</a><br>
END_OF_HTML
