...
sub dispatch(){
	...
	my ($itemCode,$itemName,$catchcopy,$itemPrice,$itemCaption,$itemUrl,$affiliateUrl,$smallImageUrl,$mediumImageUrl,$taxFlag,$postageFlag,$creditsportsdFlag,$shopOfTheYearFlag,$asurakuFlag,$reviewCount,$reviewAverage,$shopName,$shopCode,$shopUrl,$genreId,$titlestr);
	my $sth = $self->{dbi}->prepare(qq{select `itemCode`,`itemName`,`catchcopy`,`itemPrice`,`itemCaption`,`itemUrl`,`affiliateUrl`,`smallImageUrl`,`mediumImageUrl`,`taxFlag`,`postageFlag`,`creditCardFlag`,`shopOfTheYearFlag`,`asurakuFlag`,`reviewCount`,`reviewAverage`,`shopName`,`shopCode`,`shopUrl`,`genreId`,titlestr from yahoo_item_sports where itemCode = ? });
...
#	$name = $catchcopy if($catchcopy);
	...
	$title .=qq{ };
	my $keys = qq{スポーツ用品通販,送料無料,yahoo,スポーツ用品,ショッピング,スポーツ,価格,$keywords};
	my $desc = qq{$itemName };
...
my $file = qq{/var/www/vhosts/waao.jp/etc/makeshophtm/tmpl_yahoo_sports/$tmpl};
my $fh;
my $filedata;
open( $fh, "<", $file );
...
#	$name = $catchcopy if($catchcopy);
	my $title = qq{$titlestr yahooID:$itemcode スポーツ用品通販};
	my $keys = qq{スポーツ用品通販,送料無料,yahoo,スポーツ用品,価格,$keywords};
	my $desc = qq{$itemName };
