...
#	$name = $catchcopy if($catchcopy);
	...
	$title .=qq{ };
	my $keys = qq{カー用品通販,送料無料,yahoo,カー用品,ショッピング,ファッション,価格,$keywords};
	my $desc = qq{$itemName };
...
my $file = qq{/var/www/vhosts/waao.jp/etc/makeshophtm/tmpl_yahoo_car/$tmpl};
my $fh;
my $filedata;
open( $fh, "<", $file );
...
#	$name = $catchcopy if($catchcopy);
	my $title = qq{$titlestr yahooID:$itemcode カー用品通販};
	my $keys = qq{カー用品通販,送料無料,yahoo,カー用品,価格,$keywords};
	my $desc = qq{$itemName };
